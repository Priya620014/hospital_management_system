...
const AdminDashboard = () => {
  const [stats, setStats] = useState(null);
  const [doctors, setDoctors] = useState([]);
  const [analyticsData, setAnalyticsData] = useState([]); // New state for chart
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        const [statsRes, doctorsRes, analyticsRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL}/api/admin/stats`),
          fetch(`${import.meta.env.VITE_API_URL}/api/admin/doctors-list`),
  ...
  return (
    <div className="min-h-screen bg-[#f1fcfb] pt-24 px-10 pb-20">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="bg-white rounded-[3rem] p-10 shadow-xl shadow-teal-50/50 border border-slate-50">
          <div className="flex justify-between items-center mb-8 px-4">
            <div>
              <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Booking Trends</h2>
              <p className="text-slate-400 text-[10px] font-bold italic">Patient appointment volume over the last 7 days</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 bg-[#137d6e] rounded-full"></span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Daily Bookings</span>
            </div>
          </div>
              ...
              <AreaChart data={analyticsData}>
                <defs>
                  <linearGradient id="colorApt" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#137d6e" stopOpacity={0.1}/>
                    <stop offset="95%" stopColor="#137d6e" stopOpacity={0}/>
                  </linearGradient>
                </defs>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-teal-50 border border-slate-50">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setSearchTerm("")} className="bg-[#00a386] text-white px-10 py-4 rounded-full font-black uppercase text-sm hover:bg-[#008f75] transition-all">Clear Search</button>
          </div>
        </div>
