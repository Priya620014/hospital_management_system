...
// export default AddService;
import React, { useState } from "react";
import { Upload, Plus, Trash2, Save, RotateCcw, Calendar, IndianRupee, X, CheckCircle } from "lucide-react";
...
const AddService = () => {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/add-service`, {
      method: "POST",
      body: data,
    });

    if (res.ok) {
      setShowSuccess(true); // Show modern popup instead of alert
    }
  };

  return (
    <div className="min-h-screen bg-[#f1fcfb] pt-32 pb-20 px-6 relative">
      
      {/* MODERN WHITE SUCCESS POPUP */}
      {showSuccess && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center px-6">
          ...
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl relative z-10 text-center space-y-6 border border-slate-50 animate-in zoom-in duration-300">
            <div className="w-20 h-20 bg-teal-50 rounded-full flex items-center justify-center mx-auto text-[#00a386]">
              <CheckCircle size={42} />
            </div>
            <div className="space-y-2">
              <h3 className="text-2xl font-black text-slate-900 leading-tight">Service Added!</h3>
              <p className="text-slate-400 font-bold text-sm leading-relaxed px-4">
                Your new service has been successfully created and is now live.
              </p>
            </div>
            <button 
              onClick={() => setShowSuccess(false)}
              className="w-full bg-[#137d6e] text-white py-4 rounded-full font-black text-sm shadow-xl shadow-teal-100 hover:bg-[#0e5e53] transition-all active:scale-95 uppercase tracking-widest"
            >
              OK
            </button>
          </div>
